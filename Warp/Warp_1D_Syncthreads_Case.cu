...
__global__ void warp1DSyncThrKernel(const int *input, int* output)
{
...
void warp1DSyncThr(Measurements* measurements)
{
	...
	for(int i = 0; i < ITERATIONS; ++i) {
		...
		warp1DSyncThrKernel<<<GRIDSIZE, THREADBLOCKSIZE>>>(d_input, d_output);
...
int main(void)
{
	char foldName[] = "Warp_Syncthreads_1D";
	Measurements* measurements = measurements_new((char*)&foldName);	
	warp1DSyncThr(measurements);
