...
__device__ void warp1DVolatileWarp(volatile int *sdata, const unsigned int tx){
...
__global__ void warp1DVolatileKernel(const int *input, int* output)
{
	...
	if(tx < 32) {
		warp1DVolatileWarp(sdata, tx);
		if(tx == 0)
			output[bx] = sdata[0];
	}
}

void warp1DVolatile(Measurements* measurements)
{
	...
	for(int i = 0; i < ITERATIONS; ++i) {
		...
		warp1DVolatileKernel<<<GRIDSIZE, THREADBLOCKSIZE>>>(d_input, d_output);
...
int main(void)
{
	char foldName[] = "Warp_Volatile_1D";
	Measurements* measurements = measurements_new((char*)&foldName);	
	warp1DVolatile(measurements);
