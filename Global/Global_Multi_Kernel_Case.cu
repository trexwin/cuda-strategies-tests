...
__global__ void globalMultiKernelKernel(const int *input, int* output, const unsigned int inputsize)
{
...
void globalMultiKernel(Measurements* measurements)
{
	...
	for(int i = 0; i < ITERATIONS; ++i) {
        ...
        // First iteration done with d_input and d_output
        startGpuTimer(measurements);
        globalMultiKernelKernel<<<gridsize, THREADBLOCKSIZE>>>(d_input, d_output, inputsize);
        inputsize = gridsize;
        ...
        while (inputsize > 1) {
            // Subsequent iterations done with only d_output as scratchpad
            startGpuTimer(measurements);
            globalMultiKernelKernel<<<gridsize, THREADBLOCKSIZE>>>(d_output, d_output, inputsize);
            inputsize = gridsize;
            gridsize = (gridsize + THREADBLOCKSIZE - 1) / THREADBLOCKSIZE;
            recordTime(measurements, measurements->gpuFoldTime, stopGpuTimer(measurements));
        }
...
int main(void)
{
	char foldName[] = "Global_Multi_Kernel";
	Measurements* measurements = measurements_new((char*)&foldName);
	globalMultiKernel(measurements);
